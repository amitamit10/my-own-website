...
const REMBG_URL = `http://127.0.0.1:${REMBG_PORT}/api/remove?model=u2netp`;
let rembgProc = null;
  ...
  setTimeout(() => {
    waitForRembgServer()
      .then(async () => {
        ...
        await fetch(`http://127.0.0.1:${REMBG_PORT}/api/remove?model=u2netp`, { method: 'POST', body: warmForm }).catch(() => {});
